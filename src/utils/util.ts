...
export function isMobile() {
  // 正则匹配移动端特征关键词
  const mobileReg =
    /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini|Mobile|mobile|Windows Phone/i;
  // 获取浏览器的用户代理字符串
  //@ts-ignore
...
export function blobURL2Data(url: string) {
...
/** 重新设置base64的图片大小
 * 将Base64图片缩小为指定宽度(保持宽高比)
 * @param {string} base64Image - 原始Base64图片数据
 * @param {number} targetWidth - 目标宽度(默认200)
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = function () {
      try {
        ...
        // 获取压缩后的Base64
        const resizedBase64 = canvas.toDataURL("image/jpeg", 1); // 0.85是质量参数
...
export const toTime = (s: number): string => {
  ...
  const S: number | string =
    sec > 9 ? parseInt(sec + "") : "0" + parseInt(sec + "");
